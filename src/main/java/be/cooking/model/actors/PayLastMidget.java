...
class PayLastMidget extends AbstractMidget {


    public PayLastMidget(Publisher publisher, List<MessageBase> historyEvents) {
    ...
    private List<MessageBase> handleOrderPaid(OrderPaid orderPaid) {
        registerOrderAsFinished(orderPaid);
    ...
    private List<MessageBase> handleCookingTimeOutEvent(CookingTimedOut cookingTimedOut) {
        ...
        if (orderState != OrderStatus.ORDER_PLACED) {
            System.out.println("Order " + order.getOrderUUID() + " already cook " + orderState);
            return Collections.emptyList();
        } else {
            order.increaseTries();
